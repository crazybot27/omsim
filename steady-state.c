...
struct steady_state run_until_steady_state(struct solution *solution, struct board *board, uint32_t number_of_inputs, uint64_t cycle_limit)
{
    ...
    for (uint32_t i = 0; i < solution->number_of_arms; ++i) {
        ...
        if (solution->arm_tape_halt_index[i] != SIZE_MAX)
            start_cycle = solution->arm_tape_start_cycle[i] + solution->arm_tape_halt_index[i];
        uint64_t period_aligned_start_cycle = check_period * ((start_cycle + check_period - 1) / check_period);
